...
if [[ ! -d "${result_dir}/tsv" ]]; then
    python3 ./gen_tsv.py "${data_dir}/LibriSpeech" "${result_dir}/tsv" "$split"
else
...
if [[ $phase == "1" ]]; then
    ...
    if [[ ! -d "${result_dir}/mfcc" ]]; then
        python3 ./simple_kmeans/dump_mfcc_feature.py "${result_dir}/tsv" train 1 0 "${result_dir}/mfcc"
        python3 ./simple_kmeans/dump_mfcc_feature.py "${result_dir}/tsv" valid 1 0 "${result_dir}/mfcc"
    else
    ...
    if [[ ! -f "${result_dir}/phase1_kmeans.pt" ]]; then
        python3 ./simple_kmeans/learn_kmeans.py "${result_dir}/mfcc" train 1 "${result_dir}/phase1_kmeans.pt" 100 --percent -1 
    else
    ...
    if [[ ! -d "${result_dir}/phase1_labels" ]]; then
        python3 ./simple_kmeans/dump_km_label.py "${result_dir}/mfcc" train "${result_dir}/phase1_kmeans.pt" 1 0 "${result_dir}/phase1_labels"
        python3 ./simple_kmeans/dump_km_label.py "${result_dir}/mfcc" valid "${result_dir}/phase1_kmeans.pt" 1 0 "${result_dir}/phase1_labels"
    else
    ...
    if [[ ! -d "${result_dir}/hubert" ]]; then
        ...
        python3 ./simple_kmeans/dump_hubert_feature.py "${result_dir}/tsv" train "$ckpt_path" 0 1 0 "${result_dir}/hubert"
        python3 ./simple_kmeans/dump_hubert_feature.py "${result_dir}/tsv" valid "$ckpt_path" 0 1 0 "${result_dir}/hubert"
    else
    ...
    if [[ ! -f "${result_dir}/phase2_kmeans.pt" ]]; then
        python3 ./simple_kmeans/learn_kmeans.py "${result_dir}/hubert" train 1 "${result_dir}/phase2_kmeans.pt" 500 --percent -1 
    else
        echo "K-means model checkpoint already exists, skipping training process."
    fi
    ...
    if [[ ! -d "${result_dir}/phase2_labels" ]]; then
        python3 ./simple_kmeans/dump_km_label.py "${result_dir}/hubert" train "${result_dir}/phase2_kmeans.pt" 1 0 "${result_dir}/phase2_labels"
        python3 ./simple_kmeans/dump_km_label.py "${result_dir}/hubert" valid "${result_dir}/phase2_kmeans.pt" 1 0 "${result_dir}/phase2_labels"
    else
        echo "K-means labels found in ${result_dir}/phase1_labels, skipping generation process."
    fi
