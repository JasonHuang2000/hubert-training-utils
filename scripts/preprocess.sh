...
if [[ $phase == "1" ]]; then
    ...
    if [[ ! -f "${result_dir}/phase1_kmeans.pt" ]]; then
        python3 "${PROJECT_DIR}/simple_kmeans/learn_kmeans.py" "${result_dir}/mfcc" train 1 "${result_dir}/phase1_kmeans.pt" 100 --percent -1 
    else
    ...
    if [[ ! -f "${result_dir}/phase2_kmeans.pt" ]]; then
        python3 "${PROJECT_DIR}/simple_kmeans/learn_kmeans.py" "${result_dir}/hubert" train 1 "${result_dir}/phase2_kmeans.pt" 500 --percent -1 
    else
        echo "K-means model checkpoint already exists, skipping training process."
    fi
